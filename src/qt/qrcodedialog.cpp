...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("KIN")
    updateDisplayUnit();
}
...
{
    QString ret = QString("kincoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non KIN unit input in lnReqAmount, we generate the URI with KIN as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::KIN, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
